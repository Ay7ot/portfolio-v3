...
export default function Projects() {
    ...
    return (
        <section className='pt-[4rem] md:pt-[8rem] w-full max-w-[1024px]' id='projectsBody'>
            <h2 className='text-[#111322] dark:text-white text-center relative font-semibold leading-[2.375rem] text-[1.875rem] md:text-[2.75rem] lg:text-[3.75rem]'>
                Selected Projects
                <span className='text-base absolute top-[-8px] mt-[-0.5rem] max-[500px]:right-0 font-normal font-shadows lg:text-[1.75rem] md:tracking-[-1.2px] dark:text-[#B9C0D4]'>From 2021 till now</span>
            </h2>
            <div className='flex flex-col gap-8 md:gap-16 mt-8 md:mt-16' id='projects'>
                    ...
                    return (
                        <div key={index} className={`flex flex-col gap-3 md:gap-8 md:flex-row-reverse md:justify-end`}>
                            <div className={`flex items-center gap-3 ${index === 0 ? '' : index % 2 !== 0 ? 'flex-row-reverse' : 'flex-row'}`}>
                                <div className={`flex-1 md:h-[400px] h-[250px] rounded-[1.25rem] relative ${project.name === 'ResuMate' || project.name === 'BookWormz' ? ' border-[1px] border-[#DCDFEA] dark:border-none' : ''}`}>
                                    <img src={project.deskimg} className='h-full object-cover rounded-[1.5rem]' />
                                </div>
                                <div className={`flex-[0.33] md:h-[400px] h-[250px] rounded-[1.25rem] relative ${project.name === 'ResuMate' || project.name === 'BookWormz' ? ' border-[1px] border-[#DCDFEA] dark:border-none' : ''}`}>
                ...
                <button onClick={showMore ? showLessProjects : showMoreProjects} className='border-[#B9C0D4] dark:border-[#5D6B98] border-[1px] px-4 py-[10px] rounded-lg dark:bg-[#111322] hover:bg-[#F9F9FB] transition-all duration-300 ease-linear origin-left flex items-center justify-between w-full'>
